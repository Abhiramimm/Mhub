...
class Album(models.Model):
    ...
    def average_rating(self):
        ...
        if self.review_count==0:
            ...
            qs=Review.objects.filter(album_object=self).values_list("rating",flat=True)
            
            total=sum(qs)
            
            average=round((total/self.review_count),1)
    ...
    def avg_rating(self):
        ...
        return round(avg_rating,1) if avg_rating else avg_rating
